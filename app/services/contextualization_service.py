...
class ContextualizationService:
    ...
    def infer_deterministic_rewrite(messages: List[Any]) -> str:
        ...
        if not (asks_create and asks_slot):
            return ""
...
4. Keep entities, filters, and time constraints from prior turns when user is refining the same request.
