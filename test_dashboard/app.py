...
chips = "".join(
    f"<a class='suggestions-chip' href='?suggestion={quote_plus(s)}'>{s}</a>" for s in suggestions
)
st.markdown(
    f"<div class='suggestions-float'><div class='suggestions-header'>Quick Suggestions</div><div class='suggestions-list'>{chips}</div></div>",
    unsafe_allow_html=True,
)

selected_suggestion = st.query_params.get("suggestion")
prompt = selected_suggestion if selected_suggestion else st.chat_input("Ask a question...")
